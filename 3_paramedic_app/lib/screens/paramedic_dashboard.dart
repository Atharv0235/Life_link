...
class _ParamedicDashboardState extends State<ParamedicDashboard> {
  ...
  void startDrivingSimulator() {
    ...
    _driveTimer = Timer.periodic(const Duration(seconds: 1), (timer) {
      if (currentStep < greenCorridorRoute.length) {
        ...
        _mapController.move(LatLng(currentLat, currentLon), 14.5);
  ...
  Future<void> handleScanPatient() async {
    ...
    setState(() {
      isScanning = false;
      if (response['status'] == 'success') {
        triageSummary = response['ai_triage_summary'] ?? "No summary available.";
      } else {
        triageSummary = "Error: ${response['message'] ?? 'Check server connection'}";
      }
    });
  }
              ...
              Expanded(
                ...
                child: ClipRRect(
                  ...
                  child: FlutterMap(
                    ...
                    options: MapOptions(
                      initialCenter:
                          crashLocation ?? const LatLng(19.0178, 72.8478),
                      initialZoom: 14.0,
                    ),
              ...
              Expanded(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (triageSummary != "Awaiting patient scan..." && triageSummary != "Querying MediChain...") ...[
                      const SizedBox(height: 8),
                      ElevatedButton.icon(
                        icon: const Icon(Icons.arrow_forward, color: Colors.white),
                        label: const Text(
                          "CONTINUE TO AMBULANCE TRACKING",
                          style: TextStyle(color: Colors.white, fontSize: 16),
                        ),
                        style: ElevatedButton.styleFrom(
                          backgroundColor: Colors.green[700],
